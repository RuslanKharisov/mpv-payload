...
- `src/app/` - Next.js 13+ приложение с App Router
...
- `src/payload/` - конфигурация CMS
  ...
  - `access/` - правила доступа (authenticated, isSuperAdmin и др.)
...
- `src/shared/` - общие утилиты, провайдеры, UI компоненты, обеспечивающие работу личного кабинета
  ...
  - `utilities/` - служебные утилиты (getURL, generateAuthCookies и др.)
...
- Регистрация и аутентификация тенантов-поставщиков
- Управление профилем тенанта
- Каталог товаров поставщика
- Управление складскими остатками и ценами
- Функция импорта Excel для обновления остатков
- Отправка коммерческих предложений
- Приватные маршруты для личного кабинета

**В разработке:**

- Интерфейс управления заказами
- Расширенная аналитика
- Управление складами и локациями
...
## 10. Логика доступа и фильтрации
...
- Доступ к данным не фильтруется автоматически на уровне БД, а контролируется через правила доступа в каждой коллекции
...
- В процедурах не реализованы специальные проверки на принадлежность данных конкретному тенанту пользователя
